...
# --------------------------------------------------------------------------------------
# Обратное давление вместо отбрасывания.
#
# Своего topic'а мало: у LiveKit одна надёжная труба, и пинг всё равно ждёт, пока уйдут
# накопленные данные. Копятся они потому, что очередь отправки на 5000 пакетов
# принимает всё подряд, а sing-box наливает столько, сколько влезет.
#
# Лечим у истока: короткая очередь и ожидание места вместо мгновенного отказа. Тогда
# tcp наверху сам сбавляет темп (это его работа), в трубе не висит мегабайт, и пинг
# проходит за десятки миллисекунд. Скорость от этого не падает: она упирается в канал,
# а не в размер очереди.
# --------------------------------------------------------------------------------------

print("internal/engine/livekit/livekit.go (обратное давление):")
patch(
    "internal/engine/livekit/livekit.go",
    [
        (
            "\tdefaultSendQueueSize    = 5000\n\tdefaultSendQueueCapHard = 4000\n",
            "\t// Было 5000/4000. Такая очередь = мегабайты в полёте и служебный пинг,\n"
            "\t// ждущий за ними по 1-10 секунд (замер 07.08.2026). Держим её короткой:\n"
            "\t// отправитель наверху притормозит сам, а канал загружен ровно так же.\n"
            "\tdefaultSendQueueSize    = 256\n\tdefaultSendQueueCapHard = 192\n"
            "\t// Сколько ждём место в очереди, прежде чем признать отправку неудачной.\n"
            "\tsendQueueWait           = 5 * time.Second\n",
            "короткая очередь отправки",
        ),
        (
            "\tselect {\n\tcase s.sendQueue <- data:\n\t\treturn nil\n\tdefault:\n\t\treturn ErrSendQueueFull\n\t}\n}",
            "\t// Ждём место, а не отбрасываем сразу: отказ здесь приводит к ретрансмиссии\n"
            "\t// выше и к тем же данным в трубе, только позже. Ожидание — это и есть\n"
            "\t// обратное давление, ради которого очередь сделана короткой.\n"
            "\tselect {\n"
            "\tcase s.sendQueue <- data:\n"
            "\t\treturn nil\n"
            "\tcase <-s.done:\n"
