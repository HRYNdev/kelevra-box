...
# --------------------------------------------------------------------------------------
# Клиент запирал сам себя на переподключении и в комнату больше не возвращался.
#
# Замер 11.08.2026, стенд с обеих сторон под наблюдением: перезапускаем серверную ногу —
# живой клиент делает пять попыток рукопожатия по 30 секунд, все в «open control stream:
# timeout», и дальше остаётся без сессии навсегда («exhausted 5 handshake attempts»).
# Свежий процесс клиента при этом входит с первой попытки. На телефоне это выглядело так:
# комната один раз умерла — и до перезапуска приложения не воскресала.
#
# Причина найдена по коду. `reconnect()` поднимает флаг `reconnecting` и держит его до
# конца функции, а `CanSend()` при поднятом флаге отвечает «писать нельзя». Обработчик
# `onReconnect` вызывается синхронно ВНУТРИ этого окна, внутри себя открывает control-поток,
# то есть пытается писать — и упирается в собственный флаг. `muxconn.Write` крутится в
# ожидании разрешения, пока smux не оборвёт запись своим 30-секундным потолком.
#
# Лечится порядком: снимаем флаг сразу после того, как несущая собрана, и только потом
# зовём обработчик. Отложенное снятие оставляем — оно безвредно и страхует ранние выходы.
# --------------------------------------------------------------------------------------
...
patch(
    ...
    [
        (
            ...
            "\treturn nil\n"
            "}\n",
            "не запирать себя на переподключении",
        ),
    ],
)
