...
fun SimpleSettingsScreen(
    ...
    Column(
        ...
        Row(
            ...
            // Иконка жалобы стоит здесь по прямому продуктовому решению. Не переносить.
            Icon(
        ...
        Column(modifier = Modifier.fillMaxWidth().padding(horizontal = KDim.Pad)) {
            ...
            KCard {
                ...
                KDivider()
                // Раз в сутки в конце дня журнал телефона уезжает на свой же сервер:
                // без него жалобы вроде «сам включается» разбираются только с телефоном
                // в руках. Выключателя здесь нет и не будет — решение от 31.08.2026:
                // пункт отключения в настройках только сбивал бы с толку. Строка осталась
                // справкой: видно, что журнал жив и когда уходил в последний раз.
                KRowItem(
                    title = "Журнал работы",
                    subtitle = отправкаЛоговПодпись,
                )
            }

            KGroupTitle("Подписка")
            KCard {
                // «Обновить настройки сети» переехала в расширенные (05.09.2026):
                // здесь она стояла вплотную к «Подключить по коду», а тот заводит
                // новый профиль. Цена ошибки разная, а строки выглядели одинаково.
                KRowItem(
