...
object PathWords {
    ...
    fun headline(snapshot: PathSnapshot, chosen: PathId?, auto: Boolean, manualExit: String?): String = when {
        ...
        // Стоять не на чем: основной канал отказал, а дом ни на что не годен — либо
        // тоже отказал, либо его тут и быть не может (в соте дома нет). Раньше в этой
        // строке требовался именно отказ дома, и в мобильной сети, где дом больше не
        // спрашивается вовсе, «Ищу путь» пропадало в пользу «Проверяю связь».
        snapshot[PathId.MAIN].refused && !snapshot[PathId.HOME].usable -> "Ищу путь"

        else -> "Проверяю связь"
    }

    /**
     * Идёт ли замер прямо сейчас: либо путь меряется, либо про пути ещё ничего не известно.
     *
     * Живёт здесь, а не на экране, по той же причине, что и [headline]: два места с
     * одинаковым вопросом расходятся при первой же правке.
     */
    fun measuring(snapshot: PathSnapshot): Boolean =
        snapshot.anyIs(PathStatus.Probing) || !snapshot.any { it.usable || it.refused }

    /**
     * Стоит ли говорить человеку «Связи нет».
     *
     * Это итог, а не середина проверки. Пока хоть один путь меряется — рано: дом
     * отвечает первым (его сводка дешёвая), а основной канал после него меряется ещё
     * до двадцати секунд, и все двадцать экран писал «Связи нет» при живом канале
     * (жалоба владельца 10.08.2026). Порядок тот же, что у `AutoMode.linkFrom`:
     * «меряем» бьёт «не отвечает», один живой путь бьёт два отказавших.
     */
    fun linkDead(snapshot: PathSnapshot): Boolean =
        !measuring(snapshot) && !snapshot.any { it.usable } && snapshot.any { it.refused }

    /**
     * Про наборы правил — или ничего.
     *
     * Пока наборов нет, туннель поднимается, но маршруты бедные: почти всё идёт напрямую,
     * потому что правила, которые уводили трафик в туннель, ограничены наборами, а их у
     * нас ещё нет. Это заметно и это надо сказать. Молчание тут — худший вариант: человек
     * видит «Работает» и не понимает, почему работает не всё.
     *
     * @param total сколько наборов просит конфиг.
     * @param ready сколько уже лежит в кэше и ушло в ядро.
     */
    fun rulesNote(total: Int, ready: Int): String? = when {
        total <= 0 -> null
        ready <= 0 -> "правила ещё не загружены"
        ready < total -> "правила загружены не полностью ($ready из $total)"
        else -> null
    }
