...
object NetworkModeDetector {
    ...
    data class Endpoint(
         ...
         *
         * Умолчание `/` меряет мало: живьём `www.youtube.com/` отдал 3475 байт —
         * короткий редирект, честно дочитанный до конца. Для вердикта «норма» этого
         * хватает (поток закончился по-честному), а вот подвисание, которое DPI
         * устраивает «после десятков килобайт», на таком объёме не поймать —
         * до него просто не доходит. Чтобы проба ловила и это, путь должен вести
         * к телу хотя бы в [BULK_BYTES]; подобрать такой путь — работа стенда.
         */
        val path: String = "/",
    )
    ...
    data class Targets(
         ...
         *
         * ВНИМАНИЕ: [Endpoint.literals] тут пуст, и это слабое место конструкции.
         * Если оператор режет ещё и DNS, контроль не отрезолвится, и белый список
         * вместо вердикта даст [NetworkMode.Unknown]. Литерал сюда надо прописать
         * по результатам прогона на стенде — с самого стенда, а не отсюда.
         */
        ...
        companion object {
            val DEFAULT = Targets(
                ...
                allowed = Endpoint(host = "ya.ru", port = 443),
                canary = Endpoint(host = "www.youtube.com", port = 443),
            )
        }
    }
